...
export default function StoreCard({ rows, setRows, auth }) {
  ...
  const columns = [
    ...
    { field:"address", headerName:"Адреса", flex:1   },
    {
      field:"actions",
      headerName:"",
      width:110,
  ...
  return (
    <Card sx={{ height: 420,                 
      ...
      <Dialog open={openForm} onClose={()=>setOpenForm(false)}>
        <DialogTitle>{cur.id ? "Редагувати магазин" : "Додати магазин"}</DialogTitle>
        <DialogContent sx={{ display:"flex", flexDirection:"column", gap:2, mt:1 }}>
          <TextField label="Назва"   value={cur.name}
            onChange={e=>setCur({ ...cur, name:e.target.value })}/>
          <TextField label="Адреса"  value={cur.address}
            onChange={e=>setCur({ ...cur, address:e.target.value })}/>
        </DialogContent>
        <DialogActions>
